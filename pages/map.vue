  ...
  <div class="relative flex h-[calc(100vh-4rem)] w-full flex-col overflow-hidden md:h-screen">
    ...
    <transition name="fade-overlay">
      ...
      >
        ...
        <span class="animate-pulse font-mono text-xs uppercase tracking-widest text-orange-500">
          Initializing Map...
        </span>
      </div>
    </transition>
    ...
    >
      <div class="mb-2 flex items-start justify-between">
        ...
        <div class="flex items-center gap-2">
          <button
            ...
            class="rounded border border-white/15 bg-white/5 px-2 py-1 font-mono text-[10px] uppercase tracking-wide text-white/80 transition-colors hover:bg-white/10"
          >
            Locate
          </button>
          <button
            ...
            "
            class="rounded border px-2 py-1 font-mono text-[10px] uppercase tracking-wide transition-colors"
          >
            {{ autoFollow ? 'Follow ON' : 'Follow OFF' }}
          </button>
          <span
            v-if="isRecording || geoStore.isPassiveTracking"
            class="flex h-2 w-2 animate-pulse rounded-full"
            :class="isRecording ? 'bg-red-500' : 'bg-green-500'"
          ></span>
          <span class="font-mono text-[10px] uppercase opacity-50">
            {{
      ...
      <div class="space-y-1 font-mono text-[10px] leading-relaxed text-white/80">
        ...
        <div class="flex justify-between">
          ...
          <span :class="activeHeading !== null ? 'text-sky-400' : 'text-zinc-600'">
            {{
              activeHeading !== null
                ? `${Math.round(activeHeading)}° (${usedHeadingSource})`
                : 'NO_SIGNAL'
            }}
          </span>
        </div>
        ...
        <div class="mt-1 flex justify-between border-t border-white/5 pt-1">
          <span>STEPS:</span>
            ...
            <motion.div :variants="itemVariants">
              <div class="mt-2 flex flex-col space-y-1">
                ...
                <div class="nav-divider mt-2">
                  <span class="nav-divider-label">HISTORY_LOGS</span>
                </div>

                <div
                  class="custom-scrollbar mt-1 flex max-h-[200px] flex-col gap-2 overflow-y-auto pr-1"
                >
                  <div
                    v-for="r in historyRoutes"
                    :key="r.id"
                    class="group flex cursor-pointer items-center justify-between gap-2 border border-black/10 bg-black/5 p-2 transition-colors hover:border-orange-500/50"
                  ...
                  >
                    <div class="flex flex-col overflow-hidden">
                      <span class="truncate font-mono text-[10px] text-black/90">
                        LOG_{{ r.id }}
                      </span>
                      <span class="font-mono text-[8px] text-black/40">
                        {{ new Date(r.timestamp).toLocaleString() }}
                      </span>
                    </div>
                    <button
                      @click.stop="deleteRoute(r.id)"
                      class="p-1 text-red-500 opacity-0 transition-all hover:bg-red-500/10 group-hover:opacity-100"
                    >
                      <svg
                        width="12"
                        height="12"
                  ...
                  <div
                    ...
                    class="py-4 text-center font-mono text-[10px] text-black/40"
                  >
                    NO_DATA_FOUND
                  </div>
                </div>

                <div class="nav-divider mt-2"><span class="nav-divider-label">TOOLS</span></div>

                <button class="nav-btn-light" @click="recenterToCurrentLocation">⌖ Recenter</button>
                <button class="nav-btn-light" @click="fitToSelectedRoute">⛶ Fit Route</button>
                <button class="nav-btn-light" @click="requestOrientationPermission">
                  🧭 Calibrate Compass
                </button>

                <div class="nav-divider mt-2"><span class="nav-divider-label">GEOFENCES</span></div>

                <select
                  v-model.number="selectedGeofenceId"
                  class="nav-select-light"
                ...
                <input
                  ...
                  class="nav-select-light"
                />
                <label class="flex items-center gap-2 font-mono text-[10px] text-black/70">
                  <input v-model="geofenceEditor.enabled" type="checkbox" />
                  ENABLED
                </label>
                <button class="nav-btn-light" @click="createGeofenceAtCenter">
                  ＋ Add at Center
                </button>
                <button
                  class="nav-btn-light"
                ...
                <button
                  class="nav-btn-light nav-btn-light--danger"
...
const activePolyline = ref<any>(null); // separate polyline for live tracking
...
// ---- Live route drawing (ported from Map1) ----
watch(
  () => pathCoords.value,
  (coords) => {
    if (!L || !map.value || !coords?.length) return;
    const latLngs = coords.map((p: any) => L.latLng(p.lat, p.lng));
    if (activePolyline.value) {
      activePolyline.value.setLatLngs(latLngs);
    } else {
      activePolyline.value = L.polyline(latLngs, {
        color: '#f97316',
        weight: 4,
        opacity: 0.9
      }).addTo(map.value);
...
// Clear the active polyline when recording stops
watch(
  () => isRecording.value,
  (recording) => {
    if (!recording && activePolyline.value) {
...
function updateHeadingCone() {
  if (!markerIconElement.value && userMarker.value?._icon)
    markerIconElement.value = userMarker.value._icon;
...
function applySelectedGeofenceToEditor() {
  const target = geofenceList.value.find(
    (item: any) => Number(item.id) === Number(selectedGeofenceId.value)
  );
  if (!target) {
    geofenceEditor.name = '';
...
function handleUIUpdate(lat: number, lng: number, head: number | null) {
  ...
  if (autoFollow.value) {
    const shouldPan =
      !lastPanLatLng || distanceMeters(lastPanLatLng.lat, lastPanLatLng.lng, lat, lng) > 8;
    if (shouldPan) {
...
async function resolveCurrentLatLng() {
  ...
  try {
    ...
    if (import.meta.client && 'geolocation' in navigator) {
      ...
      return { lat: Number(browserPos.coords.latitude), lng: Number(browserPos.coords.longitude) };
    }
  }
...
function fitToSelectedRoute() {
  ...
  if (activePolyline.value) {
    map.value.fitBounds(activePolyline.value.getBounds(), { padding: [40, 40] });
    return;
  }
  if (userMarker.value) {
...
async function startActiveForegroundWatch() {
  ...
  try {
    ...
    activeWatchId = await Geolocation.watchPosition(
      { enableHighAccuracy: true, timeout: 15000, maximumAge: 0, minimumUpdateInterval: 1000 },
      async (position) => {
...
.nav-btn-light--danger {
  color: #dc2626;
  border-color: rgba(220, 38, 38, 0.2);
}
.nav-btn-light:disabled {
  opacity: 0.4;
  cursor: not-allowed;
}
...
:global(.loc-accuracy-ring) {
  ...
  inset: 0;
  border-radius: 50%;
  background: radial-gradient(circle, rgba(33, 150, 243, 0.18) 0%, rgba(33, 150, 243, 0) 100%);
}
:global(.loc-fan-svg) {
  position: absolute;
  inset: 0;
