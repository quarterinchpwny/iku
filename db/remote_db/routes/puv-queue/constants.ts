export const ORS_MATRIX_URL = 'https://api.openrouteservice.org/v2/matrix/driving-car';
export const ORS_WALKING_MATRIX_URL = 'https://api.openrouteservice.org/v2/matrix/foot-walking';
export const ORS_DIRECTIONS_URL = 'https://api.openrouteservice.org/v2/directions/driving-car/geojson';
export const ORS_WALKING_DIRECTIONS_URL = 'https://api.openrouteservice.org/v2/directions/foot-walking/geojson';

export const ESTIMATE_RESPONSE_CACHE_SECONDS = 60;
export const HEATMAP_RESPONSE_CACHE_SECONDS = 60 * 60;
export const ROUTES_RESPONSE_CACHE_SECONDS = 5 * 60;
