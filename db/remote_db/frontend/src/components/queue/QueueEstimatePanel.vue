<template>
  <section class="rounded-[1rem] border border-zinc-800 bg-zinc-950 p-5">
    <div class="mb-5 flex items-start justify-between gap-4">
      <div>
        <p class="text-[11px] font-semibold uppercase tracking-[0.24em] text-zinc-500">Live Estimate</p>
    ...
    <div v-else-if="estimate" class="space-y-4">
      <div class="rounded-[1rem] border border-orange-500/30 bg-orange-500/10 p-5 text-white">
        <div class="mb-4 flex items-center justify-between gap-3">
          <div>
            <p class="text-sm text-orange-100">{{ estimate.route.label }}</p>
            <h3 class="mt-1 text-3xl font-semibold capitalize">{{ estimate.message?.headline ?? estimate.level.replace('_', ' ') }}</h3>
          </div>
          <div class="rounded-lg border border-orange-400/30 bg-black/20 px-4 py-2 text-sm font-semibold">
            Score {{ estimate.score }}
          </div>
        </div>
        <p class="max-w-xl text-sm leading-6 text-orange-50">{{ estimate.message?.action ?? 'Live traffic estimate loaded.' }}</p>
      </div>

      <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
        <div class="rounded-lg border border-zinc-800 bg-[#111418] px-4 py-3">
          <p class="text-xs uppercase tracking-[0.18em] text-zinc-500">Wait estimate</p>
          <p class="mt-2 text-lg font-semibold text-white">{{ formatWaitEstimate(estimate.wait_minutes_estimate) }}</p>
        </div>
        <div class="rounded-lg border border-zinc-800 bg-[#111418] px-4 py-3">
          <p class="text-xs uppercase tracking-[0.18em] text-zinc-500">In-vehicle time</p>
          <p class="mt-2 text-lg font-semibold text-white">{{ formatDuration(estimate.signals.traffic.duration_seconds) }}</p>
        </div>
        <div class="rounded-lg border border-zinc-800 bg-[#111418] px-4 py-3">
          <p class="text-xs uppercase tracking-[0.18em] text-zinc-500">Ride total</p>
          <p class="mt-2 text-lg font-semibold text-white">{{ formatMinutes(estimate.recommendation?.ride_total_minutes) }}</p>
        </div>
        <div class="rounded-lg border border-zinc-800 bg-[#111418] px-4 py-3">
          <p class="text-xs uppercase tracking-[0.18em] text-zinc-500">Ride ETA</p>
          <p class="mt-2 text-lg font-semibold text-white">{{ formatRideEta(estimate) }}</p>
        </div>
        <div class="rounded-lg border border-zinc-800 bg-[#111418] px-4 py-3">
          <p class="text-xs uppercase tracking-[0.18em] text-zinc-500">Baseline</p>
          <p class="mt-2 text-lg font-semibold text-white">{{ formatDuration(estimate.signals.traffic.baseline_seconds) }}</p>
        </div>
        <div class="rounded-lg border border-zinc-800 bg-[#111418] px-4 py-3">
          <p class="text-xs uppercase tracking-[0.18em] text-zinc-500">Traffic ratio</p>
          <p class="mt-2 text-lg font-semibold text-white">{{ estimate.signals.traffic.ratio }}x</p>
        </div>
      </div>

      <div class="flex flex-wrap gap-2">
        <span class="rounded-lg border px-3 py-1.5 text-xs font-semibold" :class="levelClasses(estimate.level)">
          {{ estimate.level.replace('_', ' ') }}
        </span>
        <span class="rounded-lg border px-3 py-1.5 text-xs font-semibold" :class="sourceClasses(estimate.signals.traffic.source)">
        ...
        <span
          ...
          :class="estimate.meta.degraded ? 'border-rose-500/40 bg-rose-500/10 text-rose-200' : 'border-zinc-700 bg-zinc-900 text-zinc-200'"
        >
          {{ estimate.meta.degraded ? `Degraded: ${estimate.meta.degraded_reason}` : 'Healthy signal path' }}
        </span>
        <span class="rounded-lg border px-3 py-1.5 text-xs font-semibold" :class="weatherClasses(estimate.signals.weather?.severity)">
          {{ weatherTitle(estimate.signals.weather) }}
        </span>
        <span
          class="rounded-lg border px-3 py-1.5 text-xs font-semibold"
          :class="estimate.signals.calendar?.is_holiday ? 'border-orange-500/40 bg-orange-500/10 text-orange-200' : 'border-zinc-700 bg-zinc-900 text-zinc-200'"
        >
      ...
      <div v-if="estimate.message" class="grid gap-3 xl:grid-cols-[1.2fr_0.8fr]">
        <div class="rounded-[1rem] border border-zinc-800 bg-[#111418] px-5 py-4">
          <p class="text-xs uppercase tracking-[0.18em] text-zinc-500">Why this signal</p>
          <p class="mt-3 text-sm leading-6 text-zinc-100">{{ estimate.message.reason }}</p>
          <p class="mt-3 text-sm leading-6 text-zinc-300">{{ estimate.message.action }}</p>
          <p class="mt-3 text-xs font-medium uppercase tracking-[0.18em] text-orange-300">{{ estimate.message.confidence_note }}</p>
        </div>
        ...
        <div class="rounded-[1rem] border border-zinc-800 bg-[#111418] px-5 py-4">
          ...
          <p class="mt-2 text-sm leading-6 text-zinc-300">{{ estimate.recommendation?.message ?? 'No comparison available.' }}</p>
          <div class="mt-4 grid gap-3 sm:grid-cols-2">
            ...
            <div class="rounded-lg border border-zinc-800 bg-zinc-950 px-4 py-3">
              <p class="text-xs uppercase tracking-[0.18em] text-zinc-500">Walk total</p>
      ...
      <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <div class="rounded-[1rem] border border-zinc-800 bg-[#111418] px-5 py-4">
          <p class="text-xs uppercase tracking-[0.18em] text-zinc-500">Weather multiplier</p>
        ...
        <div class="rounded-[1rem] border border-zinc-800 bg-[#111418] px-5 py-4">
          <p class="text-xs uppercase tracking-[0.18em] text-zinc-500">Calendar source</p>
          <p class="mt-3 text-lg font-semibold text-white">{{ calendarTitle(estimate.signals.calendar) }}</p>
          <p class="mt-2 text-sm leading-6 text-zinc-300">{{ calendarDetail(estimate.signals.calendar) }}</p>
        </div>
        <div class="rounded-[1rem] border border-zinc-800 bg-[#111418] px-5 py-4">
          <p class="text-xs uppercase tracking-[0.18em] text-zinc-500">Incident override</p>
          <p class="mt-3 text-lg font-semibold text-white">{{ signedValue(estimate.signals.incidents?.score_delta) }}</p>
          <p class="mt-2 text-sm leading-6 text-zinc-300">{{ incidentDetail(estimate.signals.incidents) }}</p>
        </div>
        <div class="rounded-[1rem] border border-zinc-800 bg-[#111418] px-5 py-4">
          <p class="text-xs uppercase tracking-[0.18em] text-zinc-500">Observation drift</p>
          <p class="mt-3 text-lg font-semibold text-white">{{ signedValue(estimate.signals.observations?.score_delta) }}</p>
          <p class="mt-2 text-sm leading-6 text-zinc-300">{{ observationDetail(estimate.signals.observations) }}</p>
        </div>
      </div>

      <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        <div class="rounded-lg border border-zinc-800 bg-zinc-950 px-4 py-3">
          <p class="text-xs uppercase tracking-[0.18em] text-zinc-500">Cache hit</p>
          <p class="mt-1 text-sm font-medium text-white">{{ estimate.meta.cache.hit ? 'Yes' : 'No' }}</p>
        </div>
        <div class="rounded-lg border border-zinc-800 bg-zinc-950 px-4 py-3">
          <p class="text-xs uppercase tracking-[0.18em] text-zinc-500">Cache age</p>
        ...
        <div class="rounded-lg border border-zinc-800 bg-zinc-950 px-4 py-3">
          <p class="text-xs uppercase tracking-[0.18em] text-zinc-500">Commute points</p>
          <p class="mt-1 text-sm font-medium text-white">{{ Array.isArray(estimate.polyline) ? estimate.polyline.length : 0 }}</p>
        </div>
        <div class="rounded-lg border border-zinc-800 bg-zinc-950 px-4 py-3">
          <p class="text-xs uppercase tracking-[0.18em] text-zinc-500">Walking points</p>
          <p class="mt-1 text-sm font-medium text-white">{{ Array.isArray(estimate.walking_polyline) ? estimate.walking_polyline.length : 0 }}</p>
        </div>
        <div class="rounded-lg border border-zinc-800 bg-zinc-950 px-4 py-3">
          <p class="text-xs uppercase tracking-[0.18em] text-zinc-500">Computed</p>
          <p class="mt-1 text-sm font-medium text-white">{{ formatTimestamp(estimate.computed_at) }}</p>
        </div>
      </div>
    </div>

    <p v-else class="text-sm text-zinc-500">Select a route and fetch an estimate to inspect the live queue response.</p>
  </section>
</template>
...
function weatherClasses(severity) {
  return {
    clear: 'border-zinc-700 bg-zinc-900 text-zinc-200',
    light_rain: 'border-orange-500/30 bg-orange-500/10 text-orange-200',
    moderate_rain: 'border-orange-500/40 bg-orange-500/15 text-orange-100',
    heavy_rain: 'border-rose-500/40 bg-rose-500/10 text-rose-200',
  }[severity] ?? 'border-zinc-700 bg-zinc-900 text-zinc-200'
}
